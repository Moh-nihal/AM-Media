    ...
    <footer className="bg-stone-100 dark:bg-[#0A0807] border-t border-stone-200 dark:border-indigo-500/10 relative overflow-hidden">
      ...
      <div className="container-custom section-padding relative z-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-12 mb-8 md:mb-12">
          <div>
            <h3 className="text-xl sm:text-2xl font-display font-bold mb-4 bg-gradient-to-r from-indigo-500 to-rose-500 dark:from-indigo-400 dark:to-rose-400 bg-clip-text text-transparent">AL LAYAL</h3>
            <p className="text-sm sm:text-base text-stone-600 dark:text-slate-400 mb-6 leading-relaxed">From India to the heart of the GCC, AL Layal Media and Graphics connects creativity with culture — delivering premium-quality design, media, and digital solutions across Saudi Arabia, Kuwait, Oman, Qatar, Bahrain, and UAE.</p>
            <div className="flex gap-4 relative z-50 pointer-events-auto">
              {[
                { Icon: Instagram, link: 'https://www.instagram.com/allayalmedia?igsh=MTVpank4cDd6MDVqeA==', label: 'Instagram' },
          ...
          <div>
            ...
            <ul className="space-y-3">
              {['About Us', 'Services', 'Portfolio', 'Process', 'Packages', 'Contact'].map((item) => {
                const slug = item.toLowerCase().replace(' ', '')
                return (
                  <li key={item}>
                    {item === 'Portfolio' ? (
                      <Link to="/portfolio" className="text-stone-600 dark:text-slate-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors">{item}</Link>
                    ) : (
                      <a href={`#${slug}`} className="text-stone-600 dark:text-slate-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors">{item}</a>
                    )}
                  </li>
                )
              })}
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-center gap-3">
                ...
                <a href="https://wa.me/971545994816" target="_blank" rel="noopener noreferrer" className="text-stone-600 dark:text-slate-400 hover:text-rose-500 dark:hover:text-rose-400 transition-colors">+971 54 599 4816</a>
              </li>
              <li className="flex items-center gap-3">
                <Phone className="w-5 h-5 text-rose-500 dark:text-rose-400" />
                <a href="tel:+971545994816" className="text-stone-600 dark:text-slate-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors">+971 54 599 4816</a>
              </li>
              <li className="flex items-center gap-3">
                <Mail className="w-5 h-5 text-indigo-500 dark:text-indigo-400" />
                <a href="mailto:info@allayal.com" className="text-stone-600 dark:text-slate-400 hover:text-rose-500 dark:hover:text-rose-400 transition-colors">info@allayal.com</a>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="pt-6 md:pt-8 border-t border-stone-200 dark:border-indigo-500/10 text-center">
          <p className="text-stone-500 dark:text-slate-500 text-sm sm:text-base">© {currentYear} ALLAYAL MEDIA AND GRAPHICS FZE LLC. All rights reserved.</p>
        </div>
      </div>
    </footer>
  )
